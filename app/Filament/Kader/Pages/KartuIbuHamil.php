...
class KartuIbuHamil extends Page implements HasForms
{
    ...
    public function form(Form $form): Form
    {
        return $form->schema([
            Select::make('ibu_id')
                ->label('Nama Ibu')
                ->options(fn () => Ibu::where('posyandu_id', auth()->user()->posyandu_id)->whereHas('kehamilan')->pluck('nama', 'id'))
    ...
    public function getIbuData(): array
    {
        ...
        if (!$ibu) return [];
        $kehamilan = Kehamilan::with(['periksaKehamilan.kader'])->where('ibu_id', $this->ibu_id)->orderBy('hpht', 'desc')->get();
        ...
        return [
            Action::make('cetakPdf')
                ...
                ->action(function () {
                    ...
                    $pdf = Pdf::loadView('laporan.kartu-ibu-hamil-pdf', $data)->setPaper('a4', 'portrait');
                    return response()->streamDownload(fn () => print($pdf->output()), 'Kartu-Ibu-Hamil-' . str_replace(' ', '-', $data['ibu']['nama']) . '.pdf');
                }),
        ];
    }
}
