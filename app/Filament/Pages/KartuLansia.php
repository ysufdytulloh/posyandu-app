...
class KartuLansia extends Page implements HasForms
{
        ...
        return $form
            ->schema([
                Select::make('posyandu_id')
                    ...
                    ->native(false)
                    ->live()
                    ->afterStateUpdated(fn () => $this->lansia_id = null),

                Select::make('lansia_id')
                    ->label('Nama Lansia')
                    ->options(fn () =>
                        $this->posyandu_id
                            ? Lansia::where('posyandu_id', $this->posyandu_id)->pluck('nama', 'id')
                            : []
                    )
    ...
    public function getLansiaData(): array
    {
        ...
        $riwayat = PeriksaLansia::with(['kader'])
            ->where('lansia_id', $this->lansia_id)
            ->orderBy('tgl_periksa')
            ->get();
        ...
        return [
            Action::make('cetakPdf')
                ...
                ->action(function () {
                    ...
                    $pdf = Pdf::loadView('laporan.kartu-lansia-pdf', $data)
                        ->setPaper('a4', 'portrait');

                    return response()->streamDownload(
                        fn () => print($pdf->output()),
                        'Kartu-Lansia-' . str_replace(' ', '-', $data['lansia']['nama']) . '.pdf'
                    );
                }),
        ];
    }
