...
class KartuAnak extends Page implements HasForms
{
    ...
    public function getAnakData(): array
    {
        ...
        return [
            ...
            'timbang'    => \App\Models\TimbangBalita::with('hasilGizi')
                ->where('anak_id', $this->anak_id)
                ->orderBy('tgl_periksa')->get(),
            'imunisasi'  => \App\Models\Imunisasi::with('jenisImunisasi')
                ->where('anak_id', $this->anak_id)
                ->orderBy('tgl_imunisasi')->get(),
            'sdidtk'     => \App\Models\Sdidtk::where('anak_id', $this->anak_id)
                ->orderBy('tgl_periksa')->get(),
            'vitamin_a'  => \App\Models\VitaminA::where('anak_id', $this->anak_id)
                ->orderBy('tgl_distribusi')->get(),
            'obat_cacing'=> \App\Models\ObatCacing::where('anak_id', $this->anak_id)
                ->orderBy('tgl_pemberian')->get(),
            'posyandu'   => $posyandu,
            'kader'      => $kader,
        ];
    }
        ...
        return [
            Action::make('cetakPdf')
                ...
                ->action(function () {
                    ...
                    $pdf = Pdf::loadView('laporan.kartu-anak-pdf', $data)
                        ->setPaper('a4', 'portrait');

                    return response()->streamDownload(
                        fn () => print($pdf->output()),
                        'Kartu-Anak-' . str_replace(' ', '-', $data['anak']['nama']) . '.pdf'
                    );
                }),
        ];
    }
